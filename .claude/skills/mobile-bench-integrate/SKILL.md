...
| 框架 | 状态 | CMake 选项 | 模型格式 | 说明 |
|------|------|-----------|---------|------|
| **MNN** | 活跃 | `BENCHMARK_MNN=ON` | `.mnn` | git 子模块，add_subdirectory |
| **ONNX Runtime** | 活跃 | `BENCHMARK_ORT=ON` | `.onnx` | git 子模块，单独编译 |
| **ncnn** | 活跃 | `BENCHMARK_NCNN=ON` | `.param/.bin` | git 子模块 |
| **TVM** | 活跃 | `BENCHMARK_TVM=ON` | `_tvm.so` | Relax VM 编译管道 |
| **MindSpore Lite** | 活跃 | `BENCHMARK_MINDSPORE_LITE=ON` | `.ms` | 需手动下载 SDK |
| **llama.cpp** | 开发中 | `BENCHMARK_LLAMACPP=OFF` | `.gguf` | LLM 推理，git 子模块 |
| TFLite | 已停用 | `BENCHMARK_TFLITE=OFF` | `.tflite` | AAR 提取 .so |
| TNN | 已停用 | `BENCHMARK_TNN=OFF` | `.tnnproto/.tnnmodel` | git 子模块已检出 |
| QNN | 已停用 | `BENCHMARK_QNN=OFF` | QNN 格式 | 需手动下载 SDK |
...
# 推送并运行，检查精度输出
./scripts/benchmark/benchctl.sh run cnn mobilenetv2 -f <new_fw>
```
